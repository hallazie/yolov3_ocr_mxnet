...
def confidence_mask(data, threshold, train):
	bsize = BATCH_SIZE if train else 1
	thresh_np = np.zeros((bsize, 33, WIDTH//DOWNSAMPLE, HEIGHT//DOWNSAMPLE))
	thresh_np[:,4,:,:] = threshold
	thresh_in = mx.init.Constant(thresh_np.tolist())
	thresh_mt = mx.sym.Variable('mask', shape=(bsize, 33, WIDTH//DOWNSAMPLE, HEIGHT//DOWNSAMPLE), init=thresh_in)
...
def net(train):
	...
	r6 = res_block(r5, 256)
	r7 = res_block(r6, 256)
	d7 = mx.symbol.Dropout(r7, p=0.4)
	p7 = pool_block(d7)
	r8 = res_block(p7, 512)
	r9 = res_block(r8, 512)
	d9 = mx.symbol.Dropout(r9, p=0.4)
	# p9 = pool_block(r9)
	# r10 = res_block(p9, 512)
	# r11 = res_block(r10, 512)
	c12 = conv_block(d9, num_filter=33, kernel=(1,1), stride=(1,1), pad=(0,0), act_type='relu')
	msk = confidence_mask(c12, 0.5, train)
	if not train:
		return msk
...
def predict():
	...
	_, arg_params, aux_params = mx.model.load_checkpoint(model_prefix, 40)
	arg_params['mask'] = mx.nd.zeros((1,33,WIDTH//DOWNSAMPLE,HEIGHT//DOWNSAMPLE))
	...
	t = out.reshape(((WIDTH//DOWNSAMPLE)*(HEIGHT//DOWNSAMPLE)),33)
	for e in t:
		if e[4]>0:
