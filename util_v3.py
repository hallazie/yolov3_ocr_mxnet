...
def label_2_bbox(raw_shape, input_shape, label, anchors, num_class, min_down_scale, stride_down_scale):
	...
	for layer in range(len(grid_shape)):
		scale = input_shape//grid_shape[layer][0]
			...
			for y in range(cur_shape[1]):
				for anchors in range(3):
					if label[layer][x,y,anchors,4] > THRESHOLD:
						...
						xt, yt, wt, ht = label[layer][x,y,anchors,0:4]
						xt, yt, wt, ht = sigmoid(x0)+x*scale, sigmoid(y0)+y*scale, e**w0, e**h0
						idx = list(label[layer][x,y,anchors,5:]).index(1.)
						bbox[bbox_dict_reverse[idx]] = [[x0,y0],[x1,y1]]
	return bbox
...
def bbox_2_label(raw_shape, input_shape, bbox_json, anchors, num_class, min_down_scale, stride_down_scale):
	...
	for bk in bbox_json:
		...
		for layer in range(3):
			scale = input_shape[0]//grid_shape[layer][0]
		...
		xt, yt = (x0+x1)/2-cur_max_x*anchors[anchors_mask[cur_max_l][cur_max_a]][0], (y0+y1)/2-cur_max_y*anchors[anchors_mask[cur_max_l][cur_max_a]][0]
		wt, ht = math.log()
